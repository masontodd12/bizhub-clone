...
export default function SavedDealByIdPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setErr(null);
      try {
        const res = await fetch(`/api/deals/${id}`, { method: "GET" });
        if (!res.ok) throw new Error(`Not found (${res.status})`);
        const data = (await res.json()) as SavedPayload;
        if (!cancelled) setPayload(data);
      } catch (e: any) {
        if (!cancelled) setErr(e?.message ?? "Failed to load saved deal");
      } finally {
      ...
      <main className="min-h-screen bg-[#F7F8F6] text-[#111827]">
        ...
        <div className="mx-auto max-w-4xl px-6 py-10">
          <div className="rounded-2xl border border-black/10 bg-white p-6 shadow-sm">
            <div className="text-sm font-bold">Loading saved deal…</div>
          </div>
        </div>
      </main>
    );
        ...
        <div className="mx-auto max-w-4xl px-6 py-10">
          <div className="rounded-2xl border border-black/10 bg-white p-6 shadow-sm">
            ...
            <div className="mt-2 text-sm text-black/70">
              Couldn’t load deal <span className="font-semibold">{id}</span>.
            </div>
            <div className="mt-2 text-xs text-black/50">
              {err ?? "Unknown error."}{" "}
              <br />
              If you haven’t built your DB/API yet, use the local route:{" "}
              <span className="font-semibold">/saved-deal/local</span>
            </div>

            <div className="mt-5 flex gap-2">
              <Link
                href="/saved-deal/local"
                className="rounded-xl bg-[#2F5D50] px-4 py-2 text-sm font-bold text-white hover:bg-[#3F7668]"
              >
                Open Local Saved Deal
              </Link>
              <Link
    ...
    <main className="min-h-screen bg-[#F7F8F6] text-[#111827]">
      ...
      <section className="mx-auto max-w-7xl px-6 py-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <div className="mt-2 text-sm text-black/55">
              ID: <span className="font-semibold text-[#111827]">{id}</span>
              {" • "}
        ...
        <div className="mt-6 rounded-2xl border border-black/10 bg-white p-6 shadow-sm">
          ...
          {!proj?.rows?.length ? (
            <div className="mt-4 text-sm text-black/60">No projection rows found.</div>
          ) : (
