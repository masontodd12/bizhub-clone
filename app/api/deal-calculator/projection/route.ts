...
function projectYears(params: Body) {
  ...
  const paybackYears = rows[0]?.net > 0 ? params.upfrontCash / rows[0].net : null;
...
export async function POST(req: Request) {
  ...
  const access = await prisma.userAccess.findUnique({
    where: { userId },
    // IMPORTANT: include isAdmin in selection so TS/prisma knows it exists
    select: { plan: true, subscriptionStatus: true, isAdmin: true },
  });

  const ent = getEntitlements(access ?? undefined);
